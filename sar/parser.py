...
from sar import PART_CPU, PART_MEM, PART_SWP, PART_IO, PART_NET, \
    PATTERN_CPU, PATTERN_MEM, PATTERN_SWP, PATTERN_IO, PATTERN_NET, PATTERN_RESTART, \
    FIELDS_CPU, FIELD_PAIRS_CPU, FIELDS_MEM, FIELD_PAIRS_MEM, FIELDS_SWP, \
    FIELD_PAIRS_SWP, FIELDS_IO, FIELD_PAIRS_IO, FIELDS_NET, FIELD_PAIRS_NET
import mmap
import os
import re
import traceback
    ...
    def _parse_file(self, sar_parts):
        ...
        if (type(sar_parts) is ListType):
            ...
            for part in sar_parts:
                ...
                # Try to match net usage SAR file sections
            ...
            return (cpu_output, mem_output, swp_output, io_output, net_output)
            ...
            if (part_line.strip() != '') and \
                ...
                if (full_time != "Average:"):
                    ...
                    for sectionname in pairs.iterkeys():
                        ...
                        if part_type == PART_NET:
                            deviceid = elems[(1 if is_24hr is True else 2)]
                            try:
                                blah = return_dict[full_time][deviceid]
                                del(blah)
                            except KeyError:
                                return_dict[full_time][deviceid] = {}
                            return_dict[full_time][deviceid][sectionname] = \
                                value
                        else:
                            return_dict[full_time][sectionname] = value
